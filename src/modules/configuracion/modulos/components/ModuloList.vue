...
const tableHeader   = ref([
    ...
    {
        columnLabel:    'SubSección',
        columnField:    'subseccion',
        sortEnabled:    true,
        class:          "text-start"
    },
    {
...
permisos.value = store.permisos.filter((element: any) => element.codigo == '005'); // Modulo Modulos
    ...
    <div class="card-header border-0 pt-6">
        <div class="card-toolbar">
            <div class="d-flex justify-content-end" data-kt-customer-table-toolbar="base">
                <Button label='Nuevo Modulo' severity="Primary" icon="pi pi-plus" raised
                    @click="() => { router.push({name: 'modulo-nuevo', params: {id: 0}}) }"
                    v-if="sPermisos.indexOf('Nuevo') >= 0 "
                    v-tooltip-top="'Registrar un nuevo Modulo'">
                </Button>
            </div>
        </div>
        <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
                ...
                <FloatLabel variant="on">
                    ...
                    <label for="search" class="ms-10">Buscar en Modulos</label>
                </FloatLabel>
            </div>
        </div>
                        ...
                        <td v-for="col in tableHeader" :key="col.columnLabel" :class="col.class">
                            <template v-if="col.columnField === 'actions'">
                                <Button severity="info" icon="pi pi-search" raised
                                    ...
                                    @click="() => { router.push({name: 'modulo-consulta', params: {id: item.id}}) }"
                                ...
                                <Button severity="warn" icon="pi pi-pencil" raised
                                    ...
                                    @click="() => { router.push({name: 'modulo-edita', params: {id: item.id}}) }"
                                ...
                                <Button severity="danger" icon="pi pi-times" raised
                                    ...
                                    @click="() => { router.push({name: 'modulo-elimina', params: {id: item.id}}) }"
