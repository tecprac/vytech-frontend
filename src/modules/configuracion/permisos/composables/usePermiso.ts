...
const getregistro = async( id:number ):Promise<Permiso> => {
    if (id > 0){
        ApiService.setHeader();
        const { data } = await ApiService.get2(`Permisos/GetById/${id}`,null);
    
        return data;
    } else {
        const newRegistro:Permiso = {
            id: 0,
            nombre: '',
            activo: true,
        }
...
const newRegistro =async (registro:Permiso):Promise<Permiso> => {
    ApiService.setHeader();
    const { data } = await ApiService.post(`Permisos`,registro);
    return data;
};

const updateRegistro =async (registro:Permiso ):Promise<Permiso> => { 
    ApiService.setHeader();
    const { data } = await ApiService.patch(`Permisos/${registro.id}`,registro);
    return data;
};

const deleteRegistro =async ( registro:Permiso ):Promise<Permiso> => {
    ApiService.setHeader();
    const { data } = await ApiService.delete(`Permisos/${registro.id}`);
    return data;
};


const usePermiso = ( id: number) => {

    const registro      = ref<Permiso>();
