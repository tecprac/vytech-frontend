...
const tableHeader   = ref([
    ...
    {
        columnLabel:    'Modulo',
        columnField:    'conf_modulo',
        sortEnabled:    true,
        class:          "text-start"
    },
    {
        columnLabel:    'Documento',
        columnField:    'documento',
        sortEnabled:    true,
        class:          "text-start"
    },
    {
        columnLabel:    'Folio',
        columnField:    'folio_siguiente',
        sortEnabled:    true,
        class:          "text-start"
    },
    {
...
permisos.value = store.permisos.filter((element: any) => element.codigo == '041'); // Modulo Configuración Folio Documento
    ...
    <div class="card-header border-0 pt-6">
        <div class="card-toolbar">
            <div class="d-flex justify-content-end" data-kt-customer-table-toolbar="base">
                <Button label='Nuevo Folio Documento' severity="Primary" icon="pi pi-plus" raised
                    @click="() => { router.push({name: 'folio-nuevo', params: {id: 0}}) }"
                    v-if="sPermisos.indexOf('Nuevo') >= 0 "
                    v-tooltip-top="'Registrar nuevo Folio Documento'">
                </Button>
            </div>
        </div>
        <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
                ...
                <FloatLabel variant="on">
                    ...
                    <label for="search" class="ms-10">Buscar en Folios</label>
                </FloatLabel>
            </div>
        </div>
                        ...
                        <td v-for="col in tableHeader" :key="col.columnLabel" :class="col.class">
                            <template v-if="col.columnField === 'actions'">
                                <Button severity="info" icon="pi pi-search" raised
                                    ...
                                    @click="() => { router.push({name: 'folio-consulta', params: {id: item.id}}) }"
                                ...
                                <Button severity="warn" icon="pi pi-pencil" raised
                                    ...
                                    @click="() => { router.push({name: 'folio-edita', params: {id: item.id}}) }"
                                    v-tooltip.top="{ value: 'Editar', showDelay: 1000, hideDelay: 300}"
                                    class="me-3">
                                </Button>
                            </template>
                            <template v-else-if="col.columnField == 'conf_modulo'">
                                {{  item[col.columnField]!['seccion']+' '+item[col.columnField]!['subseccion']+' '+item[col.columnField]!['modulo']  }}
                            </template>
