...
permisos.value = store.permisos.filter((element: any) => element.codigo == '003'); // Modulo Departamentos
permisos.value.forEach(element => {
     sPermisos.value += element.permiso+',';
});
console.log(sPermisos.value);
    ...
    <div class="card-header border-0 pt-6">
        <div class="card-toolbar">
            <div class="d-flex justify-content-end" data-kt-customer-table-toolbar="base">
                <Button label='Nuevo Departamento' severity="Primary" icon="pi pi-plus" raised
                    @click="() => { router.push({name: 'departamento-nuevo', params: {id: 0}}) }"
                    v-if="sPermisos.indexOf('Nuevo') >= 0 "
                    v-tooltip-top="'Registrar un nuevo Departamento'">
                </Button>
            </div>
        </div>
        <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
                ...
                <FloatLabel variant="on">
                    ...
                    <label for="search" class="ms-10">Buscar en Departamentos</label>
                </FloatLabel>
            </div>
        </div>
                        ...
                        <td v-for="col in tableHeader" :key="col.columnLabel" :class="col.class">
                            <template v-if="col.columnField === 'actions'">
                                <Button severity="info" icon="pi pi-search" raised
                                    ...
                                    @click="() => { router.push({name: 'departamento-consulta', params: {id: item.id}}) }"
                                ...
                                <Button severity="warn" icon="pi pi-pencil" raised
                                    ...
                                    @click="() => { router.push({name: 'departamento-edita', params: {id: item.id}}) }"
                                ...
                                <Button severity="danger" icon="pi pi-times" raised
                                    ...
                                    @click="() => { router.push({name: 'departamento-elimina', params: {id: item.id}}) }"
