...
const tableHeader   = ref([
    ...
    {
        columnLabel:    'Rol',
        columnField:    'conf_rol',
        sortEnabled:    true,
        class:          "text-start"
    },
    {
        columnLabel:    'Departamento',
        columnField:    'conf_departamento',
        sortEnabled:    true,
        class:          "text-start"
    },
    {
...
permisos.value = store.permisos.filter((element: any) => element.codigo == '007'); // Modulo Usuarios
    ...
    <div class="card-header border-0 pt-6">
        <div class="card-toolbar">
            <div class="d-flex justify-content-end" data-kt-customer-table-toolbar="base">
                <Button label='Nuevo Usuario' severity="Primary" icon="pi pi-plus" raised
                    @click="() => { router.push({name: 'usuario-nuevo', params: {id: 0}}) }"
        ...
        <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
                ...
                <FloatLabel variant="on">
                    ...
                    <label for="search" class="ms-10">Buscar en Usuarios</label>
                </FloatLabel>
            </div>
        </div>
                        ...
                        <td v-for="col in tableHeader" :key="col.columnLabel" :class="col.class">
                            <template v-if="col.columnField === 'actions'">
                                <Button severity="info" icon="pi pi-search" raised
                                    ...
                                    @click="() => { router.push({name: 'usuario-consulta', params: {id: item.id}}) }"
                                    v-tooltip.top="{ value: 'Consultar', showDelay: 1000, hideDelay: 300}"
                                    class="me-3">
                                </Button>
                                <Button severity="warn" icon="pi pi-pencil" raised
                                    v-if="sPermisos.indexOf('Editar') >= 0 && !item['superusuario']"
                                    @click="() => { router.push({name: 'usuario-edita', params: {id: item.id}}) }"
                                    v-tooltip.top="{ value: 'Editar', showDelay: 1000, hideDelay: 300}"
                                    class="me-3">
                                </Button>
                                <Button severity="danger" icon="pi pi-times" raised
                                    v-if="sPermisos.indexOf('Eliminar') >= 0 && !item['superusuario'] "
                                    @click="() => { router.push({name: 'usuario-elimina', params: {id: item.id}}) }"
                            ...
                            <template v-else-if="col.columnField=='conf_rol'">
                                {{  item[col.columnField]['rol'] }}
                            </template>
                            <template v-else-if="col.columnField=='conf_departamento'">
                                {{  item[col.columnField]['departamento'] }}
                            </template>
                            <template v-else>
