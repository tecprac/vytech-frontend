...
const tableHeader   = ref([
    ...
    {
        columnLabel:    'A.Paterno',
        columnField:    'apaterno',
        sortEnabled:    true,
        class:          "text-start"
    },
    {
        columnLabel:    'A.Materno',
...
permisos.value = store.permisos.filter((element: any) => element.codigo == '049'); // Modulo Catalogos Agentes
    ...
    <div class="card-header border-0 pt-6">
        <div class="card-toolbar">
            <div class="d-flex justify-content-end" data-kt-customer-table-toolbar="base">
                <Button label='Nuevo Agente' severity="Primary" icon="pi pi-plus" raised
                    @click="() => { router.push({name: 'agente-nuevo', params: {id: 0}}) }"
                    v-if="sPermisos.indexOf('Nuevo') >= 0 "
                    v-tooltip-top="'Registrar un nuevo Agente'">
                </Button>
            </div>
        </div>
        <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
                ...
                <FloatLabel variant="on">
                    ...
                    <label for="search" class="ms-10">Buscar en Agentes</label>
                </FloatLabel>
            </div>
        </div>
    </div>
                        ...
                        <td v-for="col in tableHeader" :key="col.columnLabel" :class="col.class">
                            <template v-if="col.columnField === 'actions'">
                                <Button severity="info" icon="pi pi-search" raised
                                    ...
                                    @click="() => { router.push({name: 'agente-consulta', params: {id: item.id}}) }"
                                ...
                                <Button severity="warn" icon="pi pi-pencil" raised
                                    ...
                                    @click="() => { router.push({name: 'agente-edita', params: {id: item.id}}) }"
