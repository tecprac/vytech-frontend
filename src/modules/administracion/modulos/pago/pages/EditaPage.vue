...
const validarDatos = async (data: Documento) => {
    ...
    try {
        if (selectformapago.value.id == 0) {
            toast.add({ severity:   'error',summary:    'Verificar',
                detail: 'Debe seleccionar una Forma de Pago',life: 3500
            });
            return;
        }
    } catch (error) {
    ...
    if (data.importe <= 0) {
        toast.add({ severity: 'error', summary: 'Verificar',
            detail: 'El Importe del Pago debe ser mayo a 0', life: 3500 });
        return;
    }
    ...
    data.ctabanco_id        = selectcuentabanco.value.id;
            ...
            <div class="card-body border-0 pt-0 pb-0">
                <Toolbar style="border-radius: 1rem; padding: 1rem 1rem 1rem 1.5rem;" class="bg-secondary" size="small">
                    <template #start>
                        ...
                        <Button v-if="registro.id > 0" raised icon="pi pi-check" class="ms-2" severity="contrast" label="Aplicar"
                            size="small" @click="aplicar" :loading="bTimbrando"
                            :disabled="registro.estatus == 'Timbrado' || registro.estatus == 'Cancelado' || documento_detalles.length == 0">
                        </Button>
                ...
                <div class="row mt-4 mb-2">
                    ...
                    <div class="col-sm-2">
                        <DatePicker v-model="registro.fecha_pago"
                ...
                <Tabs value="0" @update:value="( value:string | number) => { tabActiva = value.toString() } ">
                    <TabList>
                        ...
                        <Tab v-if="registro.estatus == 'Timbrado'" :disabled="registro.id == 0" value="1" as="div" class="flex items-center gap-2" style="border-top-left-radius: 1rem; border-top-right-radius: 1rem;"
                    ...
                    <TabPanels>
                        <TabPanel v-if="registro.id > 0" value="0">
                            <template v-if="registro.id > 0 && registro.estatus == 'SinAplicar'">
                                <div class="row mb-4">
                                    <div class="col-sm-3">
                                        <Button label="Agregar Documento" raised icon="pi pi-file-plus"
                                            severity="success" fluid
                                            @click="openDialogDetalle(0,'Create')"
                                            :disabled="registro.estatus != 'SinAplicar'">
                                        </Button>
                                    </div>
                                </div>
                                ...
                                <Column header="ACCIONES" :pt="{ headerCell: { class: 'bg-secondary text-center'}, root: { class: 'text-center'} }">
                                    <template #body="{ data }">
                                        ...
                                        <Button v-if="registro.estatus=='SinAplicar'"
                                            ...
                                            v-tooltip.top="{ 
                                                value:      'Edita el documento del pago',
                                                showDelay:  1000,
                                            }" class="me-2"
                                            @click="openDialogDetalle(data.id,'Update')">
                                        </Button>
                                        <Button v-if="registro.estatus=='SinAplicar'"
                                            severity="danger"
                                            raised rounded size="small"
                                            icon="pi pi-trash"
                                            v-tooltip.top="{ 
                                                value:      'Eliminar el documento del pago',
                                                showDelay:  1000,
                                            }"
                        ...
                        <TabPanel v-if="registro.estatus == 'Timbrado'" value="1">
