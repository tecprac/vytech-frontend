...
<template>
        ...
        <div class="card-body border-0 pt-0 pb-0">
            <Toolbar style="border-radius: 1rem; padding: 1rem 1rem 1rem 1.5rem;" class="bg-secondary" size="small">
                <template #start>
                    ...
                    <Button v-if="registro.id > 0 && registro.estatus == 'Timbrado'" label="Movimientos"
                        size="small"  class="ms-2" raised severity="info" icon="pi pi-clipboard"
                        @click="openDialogMovimientos">
                    </Button>
                    <Button v-if="registro.id > 0 && registro.saldo > 0 && registro.estatus == 'Timbrado'" label="Aplicar Saldos"
                        size="small"  class="ms-2" raised severity="success" icon="pi pi-dollar"
                        @click="openDialogAplicaSaldos">
                    </Button>
                </template>
                <template #end>
            ...
            <Tabs  value="0" @update:value="( value:string | number) => { tabActiva = value.toString() }">
                <TabList>
                    ...
                    <Tab v-if="registro.estatus == 'Timbrado' && documento_cfdi" :disabled="registro.id == 0" value="4" as="div" class="flex items-center gap-2" style="border-top-left-radius: 1rem; border-top-right-radius: 1rem;"
    ...
    <Dialog 
        ...
        modal :closable="false" position="center"
        header="Agregar Factura para aplicar saldo"
        :style="{width: '60vw'}"
        :breakpoints="{ '960px': '75vw', '641px': '100vw' }"
        :pt = " { 
                    header: { class: 'bg-secondary' },
        ...
        <div class="row mb-2">
            ...
            <label for="importe" class="required col-form-label col-form-label-sm col-sm-2 text-primary">Aplicar Importe</label>
            <div class="col-sm-2">
                <InputNumber id="importe" v-model="documento_aplicar.aplicado" 
                    :min="0" :max="9999999.99" :max-fraction-digits="2" :min-fraction-digits="2" fluid
                    mode="currency" currency="MXN" locale="es-MX" highlight-on-focus
                    :pt="{ pcInputText: { root:{ class: 'text-end fw-bold'}}}">
                </InputNumber>
        ...
        <template #footer>
            <Button 
                ...
                severity="secondary"
                icon="pi pi-times"
                @click="closeDialogAplicarDet('cerrar')"
                :pt="{ root: { class: 'mt-2'} }">
            </Button>
            <Button 
                label="Aceptar" raised
                severity="success"
                icon="pi pi-check"
                @click="closeDialogAplicarDet('aceptar')"
                :pt="{ root: { class: 'mt-2'} }">
            </Button>
        </template>
    </Dialog> 
    <!-- DIALOGO APLICACION DE SALDOS -->
    <Dialog 
        v-model:visible="dialogAplicaSaldo"
        modal :closable="false" position="top"
        header="Aplicación de Saldo a Facturas"
        :style="{width: '60vw'}"
        :breakpoints="{ '960px': '75vw', '641px': '100vw' }"
        :pt = " { 
                    header: { class: 'bg-secondary' },
        ...
        <div class="row mt-2 mb-2">
            <div class="col-sm-3">
                <Button 
                    label="Agregar Factura" raised icon="pi pi-plus" severity="success"
                    class="ms-2" fluid
                    @click="openDialogAplicaSaldosDetalle">
                </Button>
            </div>
            <label for="saldo" class="col-form-label col-form-label-sm col-sm-2">Saldo a Aplicar</label>
            <div class="col-sm-2">
                <InputNumber v-model="registro.total" :min="0.00" :max="99999999.99" 
                    mode="currency" currency="USD" locale="en-US" fluid id="saldo"
                    :min-fraction-digits="2" :max-fraction-digits="2" disabled
                    :pt="{ pcInputText: { root:{ class: 'text-end fw-bold text-primary'}} }">
                </InputNumber>
            </div>
            <label for="pendiente" class="col-form-label col-form-label-sm col-sm-2">Saldo Disponible</label>
            <div class="col-sm-2">
                <InputNumber v-model="saldoOriginal" :min="0.00" :max="99999999.99" 
                    mode="currency" currency="USD" locale="en-US" fluid id="saldo"
                    :min-fraction-digits="2" :max-fraction-digits="2" disabled
                    :pt="{ pcInputText: { root:{ class: 'text-end fw-bold text-primary'}} }">
                </InputNumber>
        ...
        <div class="row mb-2">
            <DataTable :value="documentos_aplicar">
                <Column field="id" header="ID"
                    :pt="{ headerCell: { class: 'bg-secondary text-center'} }">
                </Column>
                ...
                <Column field="fecha" header="Fecha"
                    :pt="{ headerCell: { class: 'bg-secondary'} }">
                    <template #body="{data}">
                        {{ convertTMZdate(data.fecha) }}
                    </template>
                </Column>
                <Column field="total" header="Total" class="text-end"
                    :pt="{ headerCell: { class: 'bg-secondary'} }">
                    <template #body="{data}">
                ...
                <Column field="aplicado" header="Aplicado" class="text-end"
                    :pt="{ headerCell: { class: 'bg-secondary'} }">
                    <template #body="{data}">
                        {{ formatCurrency(data.aplicado) }}
                    </template>
                </Column>
                <Column header="ACCIONES" class="text-center"
                    ...
                    <template #body="{ data }">
                        <Button 
                            ...
                            icon="pi pi-trash"
                            v-tooltip.top="{ 
                                value:      'Eliminar el documento de la aplicación',
                                showDelay:  1000,
                            }"
                            @click="eliminaAplicado($event,data)">
                        </Button>
                    </template>
                </Column>
            </DataTable>
        ...
        <template #footer>
            <Button 
                ...
                severity="secondary"
                icon="pi pi-times"
                @click="() => { dialogAplicaSaldo = false }"
                :pt="{ root: { class: 'mt-2'} }">
            </Button>
            <Button 
                label="Aplicar" raised
                severity="success"
                icon="pi pi-check"
                @click="aplicarSaldos"
                :pt="{ root: { class: 'mt-2'} }">
