...
import type { Permisos,TipoInventario } from '../interfaces/interfaces';
import { useAuthStore } from '@/stores/auth';
import useTipoInventario from '../composables/useTipoInventario';
...
permisos.value = auth.permisos.filter((element: any) => element.codigo == '043'); // Modulo Almacén Tipo Inventario
...
const {
        ...
        deleteRegistro,
} = useTipoInventario( +route.params.id );
...
watch(isError, () => {
    if(isError.value)
        router.push({name: 'tipoinventario'})
})

const Eliminar = (data: TipoInventario) => {
    confirm.require({
...
<template> 
    ...
    <div class="card-header border-0 pt-2">
        <h2>Consulta Tipo Inventario</h2>
    </div>
    <div v-if="registro"> 
        <form @submit.prevent="Eliminar(registro!)">
            ...
            <div class="card-footer">
                <Button
                    ...
                    @click="() => { router.push({name: 'tipoinventario'})}">
                </Button>
                <Button 
