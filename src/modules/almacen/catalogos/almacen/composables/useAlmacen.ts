...
const getregistro = async( id:number ):Promise<Almacen> => {
    if (id > 0){
        ApiService.setHeader();
        const { data } = await ApiService.get2(`InvAlmacen/GetById/${id}`,null);
    
        return data;
    } else {
        const newRegistro:Almacen = {
            id:             0,
	        nombre:         '',
...
const newRegistro =async (registro:Almacen):Promise<Almacen> => {
    ApiService.setHeader();
    const { data } = await ApiService.post(`InvAlmacen`,registro);
    return data;
};

const updateRegistro =async (registro:Almacen ):Promise<Almacen> => { 
    ApiService.setHeader();
    const { data } = await ApiService.patch(`InvAlmacen/${registro.id}`,registro);
    return data;
};

const deleteRegistro =async ( registro:Almacen ):Promise<Almacen> => {
    ApiService.setHeader();
    const { data } = await ApiService.delete(`InvAlmacen/${registro.id}`);
    return data;
};
