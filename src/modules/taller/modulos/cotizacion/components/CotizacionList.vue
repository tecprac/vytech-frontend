...
const tableHeader   = ref([
    ...
    {
        columnLabel:    'Cliente',
        columnField:    'nombre_cliente',
        sortEnabled:    true,
        class:          "text-start"
    },
    {
        columnLabel:    'Atención',
        columnField:    'atencion',
        sortEnabled:    true,
        class:          "text-start"
    },
    {
...
permisos.value = store.permisos.filter((element: any) => element.codigo == '022'); // Taller->Modulos->Cotización
    ...
    <div class="card-header border-0 pt-6">
        <div class="card-toolbar">
            <div class="d-flex justify-content-end" data-kt-customer-table-toolbar="base">
                <Button label='Nueva Cotización' severity="Primary" icon="pi pi-plus" raised
                    @click="() => { router.push({name: 'cotizacion-nuevo', params: {id: 0}}) }"
                    v-if="sPermisos.indexOf('Nuevo') >= 0 "
                    v-tooltip-top="'Registrar nueva cotización '">
                </Button>
            </div>
        </div>
        <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
                ...
                <FloatLabel variant="on">
                    ...
                    <label for="search" class="ms-10">Buscar en Cotizaciones</label>
                </FloatLabel>
            </div>
        </div>
                        ...
                        <td v-for="col in tableHeader" :key="col.columnLabel" :class="col.class">
                            <template v-if="col.columnField === 'actions'">
                                <Button severity="info" icon="pi pi-search" raised
                                    ...
                                    @click="() => { router.push({name: 'cotizacion-consulta', params: {id: item.id}}) }"
                                    v-tooltip.top="{ value: 'Consultar', showDelay: 1000, hideDelay: 300}"
                                    class="me-3">
                                </Button>
                                <Button severity="warn" icon="pi pi-pencil" raised
                                    v-if="sPermisos.indexOf('Editar') >= 0 && (item.estatus == 'SinAutorizar')"
                                    @click="() => { router.push({name: 'cotizacion-edita', params: {id: item.id}}) }"
