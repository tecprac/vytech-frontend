...
const getregistro = async( id:number ):Promise<Division> => {
    if (id > 0){
        ApiService.setHeader();
        const { data } = await ApiService.get2(`Divisiones/GetById/${id}`,null);
    
        return data;
    } else {
        const newRegistro:Division = {
            id: 0,
            descripcion: '',
            activo: true,
        }
...
const newRegistro =async (registro:Division):Promise<Division> => {
    ApiService.setHeader();
    const { data } = await ApiService.post(`Divisiones`,registro);
    return data;
};

const updateRegistro =async (registro:Division ):Promise<Division> => { 
    ApiService.setHeader();
    const { data } = await ApiService.patch(`Divisiones/${registro.id}`,registro);
    return data;
};

const deleteRegistro =async ( registro:Division ):Promise<Division> => {
    ApiService.setHeader();
    const { data } = await ApiService.delete(`Divisiones/${registro.id}`);
    return data;
};
