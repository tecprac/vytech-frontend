...
const getregistro = async( id:number ):Promise<Transmision> => {
    if (id > 0){
        ApiService.setHeader();
        const { data } = await ApiService.get2(`Transmisiones/GetById/${id}`,null);
    
        return data;
    } else {
        const newRegistro:Transmision = {
            id: 0,
            descripcion: '',
            activo: true,
        }
...
const newRegistro =async (registro:Transmision):Promise<Transmision> => {
    ApiService.setHeader();
    const { data } = await ApiService.post(`Transmisiones`,registro);
    return data;
};

const updateRegistro =async (registro:Transmision ):Promise<Transmision> => { 
    ApiService.setHeader();
    const { data } = await ApiService.patch(`Transmisiones/${registro.id}`,registro);
    return data;
};

const deleteRegistro =async ( registro:Transmision ):Promise<Transmision> => {
    ApiService.setHeader();
    const { data } = await ApiService.delete(`Transmisiones/${registro.id}`);
    return data;
};


const useTransmision = ( id: number) => {

    const registro      = ref<Transmision>();
    const MensajeError  = ref<string>('');
    const toast         = useToast();

    const { isPending, data, isError } = useQuery({
        queryKey:    ['transmision', id],
