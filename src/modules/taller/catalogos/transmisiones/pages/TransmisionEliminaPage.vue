...
import useTransmision from '../composables/useTransmision';
...
const {
        ...
        isDeleting,
        isDeletingSuccess,
} = useTransmision( +route.params.id );
...
watch(isError, () => {
    if(isError.value)
        router.replace('/transmisiones')
})

const Eliminar = (registro: Transmision) => {
    confirm.require({
        ...
        <form @submit.prevent="Eliminar(registro!)">
            ...
            <div class="card-footer">
                <Button
                    ...
                    @click="() => { router.push({name: 'transmisiones'})}">
                </Button>
                <Button
