...
const getregistro = async( id:number ):Promise<Bahia> => {
    if (id > 0){
        ApiService.setHeader();
        const { data } = await ApiService.get2(`Bahias/GetById/${id}`,null);
    
        return data;
    } else {
        const newRegistro:Bahia = {
            id: 0,
            descripcion: '',
...
const newRegistro =async (registro:Bahia):Promise<Bahia> => {
    ApiService.setHeader();
    const { data } = await ApiService.post(`Bahias`,registro);
    return data;
};

const updateRegistro =async (registro:Bahia ):Promise<Bahia> => { 
    ApiService.setHeader();
    const { data } = await ApiService.patch(`Bahias/${registro.id}`,registro);
    return data;
};

const deleteRegistro =async ( registro:Bahia ):Promise<Bahia> => {
    ApiService.setHeader();
    const { data } = await ApiService.delete(`Bahias/${registro.id}`);
    return data;
};


const useBahia = ( id: number) => {

    const registro      = ref<Bahia>();
