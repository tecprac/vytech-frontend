...
const getregistro = async( id:number ):Promise<Motivo> => {
    if (id > 0){
        ApiService.setHeader();
        const { data } = await ApiService.get2(`Motivo/GetById/${id}`,null);
    
        return data;
    } else {
        const newRegistro:Motivo = {
            id:             0,
	        descripcion:    '',
	        activo:         true,
        }
...
const newRegistro =async (registro:Motivo):Promise<Motivo> => {
    ApiService.setHeader();
    const { data } = await ApiService.post(`Motivo`,registro);
    return data;
};

const updateRegistro =async (registro:Motivo ):Promise<Motivo> => { 
    ApiService.setHeader();
    const { data } = await ApiService.patch(`Motivo/${registro.id}`,registro);
    return data;
};

const deleteRegistro =async ( registro:Motivo ):Promise<Motivo> => {
    ApiService.setHeader();
    const { data } = await ApiService.delete(`Motivo/${registro.id}`);
    return data;
};
