...
import type { TipoUnidad } from '../interfaces/interfaces';
import { useToast } from 'primevue/usetoast';

const getregistro = async( id:number ):Promise<TipoUnidad> => {
    if (id > 0){
        ApiService.setHeader();
        const { data } = await ApiService.get2(`TipoUnidad/GetById/${id}`,null);
    
        return data;
    } else {
        const newRegistro:TipoUnidad = {
            id: 0,
            descripcion: '',
            activo: true,
        }
...
const newRegistro =async (registro:TipoUnidad):Promise<TipoUnidad> => {
    ApiService.setHeader();
    const { data } = await ApiService.post(`TipoUnidad`,registro);
    return data;
};

const updateRegistro =async (registro:TipoUnidad ):Promise<TipoUnidad> => { 
    ApiService.setHeader();
    const { data } = await ApiService.patch(`TipoUnidad/${registro.id}`,registro);
    return data;
};

const deleteRegistro =async ( registro:TipoUnidad ):Promise<TipoUnidad> => {
    ApiService.setHeader();
    const { data } = await ApiService.delete(`TipoUnidad/${registro.id}`);
    return data;
};


const useTipoUnidad = ( id: number) => {

    const registro      = ref<TipoUnidad>();
    const MensajeError  = ref<string>('');
    const toast         = useToast();

    const { isPending, data, isError } = useQuery({
        queryKey:    ['tipounidad', id],
