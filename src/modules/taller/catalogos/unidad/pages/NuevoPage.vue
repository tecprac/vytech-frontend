...
const schema = yup.object({
    ...
    niv:            yup.string().notRequired().label('VIN'),
    ...
    clase:          yup.string().notRequired().max(10).label('Clase de la Unidad'),
    ejes:           yup.number().notRequired().positive().label('Numero de ejes'),
    num_transmision:yup.string().notRequired().max(50).label('Numero de serie de la Transmisión'),
    combustible:    yup.string().required().min(1).max(15).label('Combustible'),
    tarjetacircula: yup.string().notRequired().max(20).label('Tarjeta de Circulación'),
    pdftcirculacion:yup.string().notRequired(),
    pdfpermisosct:  yup.string().notRequired(),
    estatus:        yup.string().required().min(1).max(15).label('Estatus'),
    permiso_sct:    yup.string().notRequired().max(30).label('Permiso SCT'),
    polizaseguro:   yup.string().notRequired().max(30).label('Póliza de Seguro'),
    aseguradora:    yup.string().notRequired().max(30).label('Aseguradora'),
    vigenciapoliza: yup.date().notRequired(),
    arrendado:      yup.boolean().required().label('Arrendado'),
    pdfpolizaseguro:yup.string().notRequired(),
