...
const schema = yup.object({
    ...
    placas:         yup.string().optional().min(1).max(10).label('Placas'),
    kms:            yup.number().optional().moreThan(-1).label('Kilometraje'),
    niv:            yup.string().optional().label('VIN'),
    num_motor:      yup.string().optional().min(1).max(30).label('Numero de Motor'),
    color:          yup.string().optional().min(1).max(20).label('Color'),
    num_serie:      yup.string().optional().min(1).max(30).label('Numero de Serie Motor'),
    clase:          yup.string().optional().min(1).max(10).label('Clase de la Unidad'),
    ejes:           yup.number().optional().positive().label('Numero de ejes'),
    num_transmision:yup.string().optional().min(1).max(50).label('Numero de serie de la Transmisión'),
    combustible:    yup.string().required().min(1).max(15).label('Combustible'),
    tarjetacircula: yup.string().optional().min(1).max(20).label('Tarjeta de Circulación'),
    pdftcirculacion:yup.string().optional(),
    pdfpermisosct:  yup.string().optional(),
    estatus:        yup.string().required().min(1).max(15).label('Estatus'),
    permiso_sct:    yup.string().optional().min(1).max(30).label('Permiso SCT'),
    polizaseguro:   yup.string().optional().min(1).max(30).label('Póliza de Seguro'),
    aseguradora:    yup.string().optional().min(1).max(30).label('Aseguradora'),
    vigenciapoliza: yup.date().optional(),
    arrendado:      yup.boolean().required().label('Arrendado'),
    pdfpolizaseguro:yup.string().optional(),
...
const [fecha_compra]    = defineField('fecha_compra');
const [año]             = defineField('año');
const [placas]          = defineField('placas');
const [kms]             = defineField('kms');
const [niv]             = defineField('niv');
const [num_motor]       = defineField('num_motor');
const [color]           = defineField('color');
const [num_serie]       = defineField('num_serie');
const [clase]           = defineField('clase');
const [ejes]            = defineField('ejes');
const [num_transmision] = defineField('num_transmision');
const [combustible]     = defineField('combustible');
const [tarjetacircula]  = defineField('tarjetacircula');
const [estatus]         = defineField('estatus');
const [permiso_sct]     = defineField('permiso_sct');
const [polizaseguro]    = defineField('polizaseguro');
const [aseguradora]     = defineField('aseguradora');
const [vigenciapoliza]  = defineField('vigenciapoliza');
const [arrendado]       = defineField('arrendado');
const [pdfpolizaseguro] = defineField('pdfpolizaseguro');
const [pdftcirculacion] = defineField('pdftcirculacion');
const [pdfpermisosct]   = defineField('pdfpermisosct');

setFieldValue('combustible','Diesel');
setFieldValue('estatus','Activo');
setFieldValue('arrendado',false);
setFieldValue('kms',0);
...
const onSubmit = handleSubmit( async (values) => {
    ...
    registro.value = {
        ...
        clase:              values.clase,
        ejes:               values.ejes,
        num_transmision:    values.num_transmision,
        combustible:        values.combustible,
        tarjetacircula:     values.tarjetacircula,
        pdftcirculacion:    values.pdftcirculacion,
        pdfpermisosct:      values.pdfpermisosct,
        estatus:            values.estatus,
        permiso_sct:        values.permiso_sct,
        polizaseguro:       values.polizaseguro,
        aseguradora:        values.aseguradora,
        vigenciapoliza:     values.vigenciapoliza,
        arrendado:          values.arrendado,
        pdfpolizaseguro:    values.pdfpolizaseguro,
        activo: true
    };
            ...
            <div class="card-body border-0 pt-0 pb-0">
                ...
                <div class="row mb-2">
                    <div class="col-sm-4 fv-row">
                        ...
                        <DatePicker v-model="fecha_compra"
                            show-icon :show-on-focus="false" fluid>
                        </DatePicker>
                    </div>
                    <div class="col-sm-4 fv-row">
                        <label for="año" class="form-label">Año</label>
                        <InputMask v-model="año" mask="9999" :unmask="true" placeholder="0000" fluid
                            :pt="{ pcInputText: { root:{ class: 'text-end'}} }">
                        </InputMask>
                    </div>
                    <div class="col-sm-4 fv-row">
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-4 fv-row">
                        ...
                        <InputText v-model="num_transmision" fluid>
                        </InputText>
                    </div>
                </div>
                <div class="row mb-5">
                        ...
                        <TabPanel value="0">
                            <div class="row mb-2">
                                <div class="col-sm-4 fv-row">
                                    ...
                                    <InputText v-model="aseguradora" fluid>
                                    </InputText>
                                </div>
                                <div class="col-sm-4 fv-row">
                                    <label for="poliza" class="form-label">N° Póliza</label>
                                    <InputText v-model="polizaseguro" fluid>
                                    </InputText>
                                </div>
                                <div class="col-sm-4 fv-row">
                                    <label for="vigencia" class="form-label">Vigencia Póliza</label>
                                    <DatePicker v-model="vigenciapoliza" fluid show-icon :show-on-focus="false">
                                    </DatePicker>
                                </div>
                            </div>
                            <div class="row mb-5">
                                <div class="col-sm-4 fv-row">
                                    ...
                                    <InputText v-model="tarjetacircula" fluid>
                                    </InputText>
                                </div>
                                <div class="col-sm-4 fv-row">
                                    <label for="permisosct" class="form-label">Permiso SCT</label>
                                    <InputText v-model="permiso_sct" fluid>
                                    </InputText>
                                </div>
                                <div class="col-sm-4 fv-row">
