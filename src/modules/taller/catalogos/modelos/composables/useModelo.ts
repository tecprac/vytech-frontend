...
const getregistro = async( id:number ):Promise<Modelo> => {
    if (id > 0){
        ApiService.setHeader();
        const { data } = await ApiService.get2(`Modelos/GetById/${id}`,null);
    
        return data;
    } else {
        const newRegistro:Modelo = {
            id: 0,
            descripcion: '',
            activo: true,
        }
...
const newRegistro =async (registro:Modelo):Promise<Modelo> => {
    ApiService.setHeader();
    const { data } = await ApiService.post(`Modelos`,registro);
    return data;
};

const updateRegistro =async (registro:Modelo ):Promise<Modelo> => { 
    ApiService.setHeader();
    const { data } = await ApiService.patch(`Modelos/${registro.id}`,registro);
    return data;
};

const deleteRegistro =async ( registro:Modelo ):Promise<Modelo> => {
    ApiService.setHeader();
    const { data } = await ApiService.delete(`Modelos/${registro.id}`);
    return data;
};
