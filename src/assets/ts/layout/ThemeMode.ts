...
class ThemeMode {
    ...
    public getMode = (): Mode => {
        ...
        if (ls) { return ls as Mode; }

        const dataTheme = this.element?.getAttribute("data-bs-theme");
        if (dataTheme) { return dataTheme as Mode; }

        if (!themeMode) { return defaultMode; }

        if (themeMode === "system") { return this.getSystemMode();}
    ...
    public getMenuMode = (): Mode | "" => {
        ...
        if (dataKTValue) { return dataKTValue as Mode; }

        if (!menuModeParam) { return ""; }
